...
trait SimpleSchemaGenerator {
  implicit def intAvroSchema[T <: Int]: SchemaGenerator[T] = new SchemaGenerator[T] {
    override def generate = JString("int")
  }

  implicit def longAvroSchema[T <: Long]: SchemaGenerator[T] = new SchemaGenerator[T] {
    override def generate: JValue = JString("long")
  }

  implicit def doubleAvroSchema[T <: Double]: SchemaGenerator[T] = new SchemaGenerator[T] {
    override def generate: JValue = JString("double")
  }

  implicit def floatAvroSchema[T <: Float]: SchemaGenerator[T] = new SchemaGenerator[T] {
    override def generate: JValue = JString("float")
  }

  implicit def stringAvroSchema[T <: String]: SchemaGenerator[T] = new SchemaGenerator[T] {
  ...
  implicit val instantSchema: SchemaGenerator[Instant] = new SchemaGenerator[Instant] {
    override def generate: JValue = longAvroSchema[Long].generate
  }
